...
**P0（可验收，默认双机 `.101`+`.53`）：**
1. **SB 稳态**：禁暗 keep / 禁 hasColor→KeepEnable；停脚本必拆 keep；miss 熔断；长跑无 SB 环（对标 `.171`）
2. **找色内存模型**：显式 `keepScreen(true)`=锁当前前台缓冲；默认不 keep；@3x TTL；进程预算见上
3. **色点与引擎解耦**：引擎 PASS=HOT20+keep 态+无 SB 重启；业务 PASS=同屏 getColor 对链后再跑 Desktop 脚本

**P1（P0 绿之后）：** 单进程合并 framecap+lua（零热路径文件 IPC）；SB dylib 减面  
**P2：** 控制面全量 shm；NCNN 等加速
