...
> **架构铁律（2026-07-31 起写死）**  
> 1）兼容 **7/7P/8/8P × iOS 13～17**（用户 2026-09-11 明确：最终必须全通用；测试机只是抽样，矩阵未覆盖格子不得当 PASS） 
...
| 规则 | 说明 |
|------|------|
| **自测前清机（铁律）** | 每次五机自测前必须：停脚本、杀业务/游戏进程、收僵尸、释帧压内存、保证 `FC_N=1`。脚本：`bash tools/zy_pretest_clean_4phone.sh`。不清不测。 |
| 机型矩阵 | 必须支持越狱 iPhone **7 / 7P / 8 / 8P**，系统 **iOS 13～17**；`.61` 的 iPhone 7 / iOS 15.8.8 rootless 是强制验收点；**禁止单机死坐标** |
| **通用性验收（铁律）** | 业务脚本必须**同一份字节**在矩阵内所有真机上、从冷态由脚本自身跑通；门禁 `bash tools/zy_universal_script_gate.sh <script.lua> <bid> [devices]`。判据：每机远端脚本 sha256 与本地一致 + 目标 App 连续 3 次在前台。先人工摆好前台/先开 App 再跑**不计入**。2026-09-11 实测：`ios7.lua` 写死桌面图标坐标 (1010,294)，在 iOS 13 三机命中、在 iOS 15 的 `.61`（首页多两个小组件共 6 格）落到壁纸 → 脚本永远进不了游戏 |
...
| 测试机专用 | `.53`/`.101`/`.112`/`.166`/`.61` **只做本项目测试**；唯一部署与验收结论来源 |
| 超越目标 | 综合性能完全超越触动，尤其 SB 稳定性与业务脚本执行效率 |
| 结论隔离 | TouchSprite 观察结果 ≠ 子砚功能验证结论 |
| 禁止混用 | 不得在观察设备上部署/验收子砚；不得把观察设备冒烟写成子砚 PASS |
| 主计划真机 | `.61` 已提升为主计划第五台 rootless 真机；必须与原四机一起取得当前包、脚本、资源、run_id 和设备端 final verdict |
...
|----|------|
| 设备 IP | 192.168.31.61 |
| 机型 | **iPhone 7**（ProductType `iPhone9,1`） |
| 系统 | iOS **15.8.8** |
| 环境 | rootless（检测到 `/var/jb`） |
| SSH | 已验证可达；用户 `mobile`（UID 501）；仅执行只读探测 |
| 设备角色 | ZiYan 主计划第五台 rootless 兼容性验收设备 |
| 当前状态 | `SSH_REGISTERED`；尚未安装 ZiYan、尚未执行功能验收 |
| 门禁关系 | 纳入 `.101 -> .112 -> .166 -> .53 -> .61` 主计划顺序；缺 `.61` final verdict 时主计划门禁保持未完成 |
| 下一步 | 使用 rootless 包、脚本、资源和清理合同执行预检、安装、样本矩阵和 final verdict |
