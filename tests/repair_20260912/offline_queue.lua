-- lua/modules 也入 package.path：2026-09-12 去重后 OfflineQueue 用 require('zy_shell')
-- 载入同目录正本，只给 'lua/?.lua' 会找不到（设备上是绝对安装路径兜底）。
package.path='lua/?.lua;lua/modules/?.lua;'..package.path
...
os.execute=function(cmd)
  -- 2026-09-12 去重后 zy_shell 统一走 sh -c '…' 并把内层单引号转义成 '\''。
  -- 真机上这是正确的 POSIX 写法；本桩按字符串解析命令，所以先还原成裸命令。
  cmd=cmd:gsub("'\\''","'")
