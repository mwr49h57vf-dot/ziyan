...
# force fresh script if /var/root copy exists
[ -f /var/root/login_lan.lua ] && cp -f /var/root/login_lan.lua "$SCRIPT" && chown mobile:mobile "$SCRIPT"
...
if ! ps -A | grep -v grep | grep -q '[Z]iYan.app/ZiYan'; then
  uiopen /Applications/ZiYan.app >/dev/null 2>&1 || true
  sleep 3
fi
...
echo '--- round ---'
cat "$VAR/.ziyan_login_lan_round.txt" 2>/dev/null || echo '(none)'
echo '--- orient ---'
...
for w in 1 2 3 4 5 6; do
  r=$(sed -n 's/.*round=\([0-9]*\).*/\1/p' "$VAR/.ziyan_login_lan_round.txt" 2>/dev/null | head -1)
  [ -n "$r" ] && [ "$r" -ge 2 ] && break
  sleep 1
done
r=$(sed -n 's/.*round=\([0-9]*\).*/\1/p' "$VAR/.ziyan_login_lan_round.txt" 2>/dev/null | head -1)
