#!/bin/bash
# C87 Gate C 30-minute detached runner for .166 C90 Gate-C 30m (real Home each minute).
...
OUT="${1:-$ROOT/tmp_shots/P2_30M_C90_$(date +%Y%m%d_%H%M%S)}"
...
sample_101() {
  ssh_one 192.168.31.166 "EXPECTED_COLOR='$EXPECTED_COLOR' EXPECTED_APP='$EXPECTED_APP' bash -s" <<'REMOTE'
set +e
