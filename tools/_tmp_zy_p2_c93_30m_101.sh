#!/bin/bash
# C87 Gate C 30-minute detached runner for .101 C93 Gate-C 30m (real Home each minute).
...
OUT="${1:-$ROOT/tmp_shots/P2_30M_C93_101_$(date +%Y%m%d_%H%M%S)}"
...
ssh_one() {
  local ip="$1"
  shift
  # ConnectTimeout 略加长；完整采样重试在 minute 循环（可重抛 heredoc）。
...
sample_101() {
  ssh_one 192.168.31.101 "EXPECTED_COLOR='$EXPECTED_COLOR' EXPECTED_APP='$EXPECTED_APP' bash -s" <<'REMOTE'
set +e
...
while test "$minute" -le "$EXPECTED_MINUTES"; do
  ...
  while test "$ssh_try" -le 3; do
    ...
    zy_rc=$?
    zy="$zy
ZY101 SSH_FAIL RC=$zy_rc try=$ssh_try"
    # 仅 SSH 闪断重采；若已有 PASS/FAIL 行则不再重试
    echo "$zy" | grep -q 'ZY101 PASS=' && break
    sleep $((ssh_try * 3))
    ssh_try=$((ssh_try + 1))
  done
  if test "$zy_rc" != 0 && ! echo "$zy" | grep -q 'ZY101 PASS='; then
    zy="$zy
