#!/bin/bash
# C87 Gate C 30-minute detached runner for .101 (real Home each minute).
...
OUT="${1:-$ROOT/tmp_shots/P2_30M_C87_$(date +%Y%m%d_%H%M%S)}"
...
ssh_one() {
  local ip="$1"
  shift
  # ConnectTimeout 略加长；完整采样重试应在 minute 循环重抛 heredoc。
